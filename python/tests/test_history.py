...
def test_clamp_history_retention_days() -> None:
    assert main.clamp_history_retention_days(7) == 7
    assert main.clamp_history_retention_days(0) == 1
    assert main.clamp_history_retention_days(999) == 90
    assert main.clamp_history_retention_days("nope") == 7


def test_set_history_retention_days(write_sample_config: Path) -> None:
    assert main.set_history_retention_days(14) == 14
    raw = json.loads(write_sample_config.read_text(encoding="utf-8"))
    assert raw["history_retention_days"] == 14
    assert main.load_config().history_retention_days == 14
...
def test_prune_history_cuts_old_segments() -> None:
    ...
    pruned = main.prune_history(history, 7, now=now)
...
def test_get_peer_history_respects_retention(tmp_app_dir: Path) -> None:
    main.save_default_config()
    main.set_history_retention_days(1)
    now = datetime.now()
    ...
    segs = main.get_peer_history("26.0.0.2")
    assert len(segs) == 1
